...
async function recordMainPageWithPauses(url, outputPath) {
  ...
  // PHASE 1: 3-second header pause (72 frames)
  console.log('⏸️  Capturing 3-second header pause...');
  const headerPauseFrames = 72;
  ...
  for (let i = 0; i < headerPauseFrames; i++) {
    ...
    if (i % 24 === 0) console.log(`  Header: ${i}/${headerPauseFrames}`);
  }
  ...
  const scrollFrames = Math.floor(scrollDuration * 24); // 24 FPS
  ...
  for (let i = 0; i < scrollFrames; i++) {
    ...
    if (i % 50 === 0) console.log(`  Scroll progress: ${i}/${scrollFrames} frames`);
  }
  
  console.log(`✅ Captured ${scrollFrames} scroll frames`);
  
  // PHASE 3: 2-second bottom pause (48 frames)
  console.log('⏸️  Capturing 2-second bottom pause...');
  const bottomPauseFrames = 48;
  ...
  for (let i = 0; i < bottomPauseFrames; i++) {
    ...
    if (i % 24 === 0) console.log(`  Bottom: ${i}/${bottomPauseFrames}`);
  }
  
  console.log(`✅ Total frames: ${frameIndex}`);
  
  // Create video with ULTRAFAST preset
  console.log('🎬 Creating video...');
  const inputPattern = path.join(tempFrameDir, 'frame_%05d.jpg');
  await execAsync(`ffmpeg -y -r 24 -i "${inputPattern}" -c:v libx264 -pix_fmt yuv420p -crf 23 -preset ultrafast "${outputPath}"`);
...
async function recordNavPageFast(url, outputPath) {
  ...
  for (let i = 0; i < totalFrames; i++) {
    ...
    if (i % 50 === 0) console.log(`  Progress: ${i}/${totalFrames} frames`);
  }
